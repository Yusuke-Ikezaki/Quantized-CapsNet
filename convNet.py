...
class ConvNet(object):
    ...
    def build_arch(self):
        with tf.variable_scope('Conv1_layer'):
            ...
            conv1 = tf.nn.conv2d(self.X, W1, strides=[1, 1, 1, 1], padding='SAME') + biases1
            assert conv1.get_shape() == [cfg.batch_size, 20, 20, 32]
        ...
        with tf.variable_scope('Conv2_layer'):
            ...
            # conv1 = tf.contrib.layers.conv2d(self.X, num_outputs=32, kernel_size=5, stride=1, padding='SAME')
            conv2 = tf.nn.conv2d(pool1, W2, strides=[1, 1, 1, 1], padding='SAME') + biases2
            assert conv2.get_shape() == [cfg.batch_size, 12, 12, 64]
        ...
        with tf.variable_scope('FC1_layer'):
            ...
            fc1 = tf.matmul(flatten, W3) + biases3
        ...
        with tf.variable_scope('FC2_layer'):
            ...
            self.softmax_v = tf.nn.softmax(tf.matmul(fc1, W4) + biases4)
            assert fc2.get_shape() == [cfg.batch_size, 10]

        self.argmax_idx = tf.to_int32(tf.argmax(fc2, axis=1))
        assert self.argmax_idx.get_shape() == [cfg.batch_size]
        # self.argmax_idx = tf.reshape(self.argmax_idx, shape=(cfg.batch_size, ))

    def loss(self):
        self.total_loss = -tf.reduce_sum(self.Y * tf.log(self.softmax_v))
